...
#   described above.
# - the functions from workflow/lib/log.sh
#
...
_setup_env() {
  ...
  export CERTS_DIR="${REPO_ROOT}/bootstrap/cluster.bootstrap/structure.app/scripts/secrets/certs"
  export KUBECONF_DIR="${HOME}/.kube"

  if ! source workflow/lib/log.sh; then
